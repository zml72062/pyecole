...
class Dynamics(Protocol[Action, ActionSet]):
    ...
    def set_dynamics_random_state(
        self, model: Model, rng: RandomEngine
    ) -> None:
    ...
    def reset_dynamics(self, model: Model) -> Tuple[bool, ActionSet]:
        """Start a new episode.
    ...
    def step_dynamics(self, model: Model, action: Action) -> Tuple[bool, ActionSet]:
...
class DataFunction(Protocol[Data]):
    ...
    def before_reset(self, model: Model) -> None:
        ...
        ----------
        model:
            The :py:class:`~Model`, model defining the current state of the solver.

        """
        raise NotImplementedError()

    def extract(self, model: Model, done: bool) -> Data:
...
class ObservationFunction(DataFunction[Observation], Protocol[Observation]):
    ...
    def extract(self, model: Model, done: bool) -> Observation:
        raise NotImplementedError()
...
class InformationFunction(DataFunction[Dict[str, Information]], Protocol[Information]):
    ...
    def extract(self, model: Model, done: bool) -> Dict[str, Information]:
        raise NotImplementedError()
...
class InstanceGenerator(Protocol):
    ...
    The class combines a :py:class:`~RandomEngine` with the static function :py:meth:`generate_instance`
    to provide iterating capabilities.
    """

    @staticmethod
    def generate_instance(
        *args: Any, rng: RandomEngine, **kwargs: Any
    ) -> Model:
        """Generate a problem instance using the random generator for any source of randomness."""
        raise NotImplementedError()

    @overload
    def __init__(self, *args: Any, rng: RandomEngine, **kwargs: Any) -> None:
